...
struct index {
    ...
    void pseudoalign_threshold_union(std::string const& sequence, std::vector<std::vector<uint32_t>>& results,
                                     const double threshold) const; //modification: alternative for tsv output format
    void intersect_unitigs(std::vector<uint32_t>& unitig_ids,
